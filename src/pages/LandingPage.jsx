...
function AuthForm({ isLogin, setIsLogin, setShowAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin && !agreedToTerms) {
      toast.error(language === 'ar' ? 'يجب الموافقة على سياسة الخصوصية وشروط الخدمة.' : 'You must agree to the Privacy Policy and Terms of Service.');
      return;
    }
    setLoading(true);
    try {
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12 relative z-10">
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!isLogin && (
              <div className="flex items-start gap-3 space-y-0">
                <Checkbox
                  ...
                  required
                />
                <Label htmlFor="agreeTerms" className="text-sm leading-relaxed cursor-pointer text-muted-foreground">
                  {language === 'ar' ? (
                    <>أوافق على <Link to="/terms" target="_blank" className="text-primary hover:underline">شروط الخدمة</Link> و<Link to="/privacy" target="_blank" className="text-primary hover:underline">سياسة الخصوصية</Link>.</>
                  ) : (
            ...
            <Button type="submit" className="w-full h-12 rounded-xl font-semibold text-base" disabled={loading || (!isLogin && !agreedToTerms)}>
