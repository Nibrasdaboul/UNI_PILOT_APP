  ...
  const fetchCourses = async () => {
    try {
      const [coursesRes, catalogRes] = await Promise.all([
        api.get('/student/courses?semester=current'),
    ...
    <div className="space-y-8 pb-12" data-testid="courses-page">
      ...
      <div>
        <h3 className="text-xl font-bold font-display mb-4">
          {language === 'ar' ? 'موادي المسجلة (الفصل الحالي)' : 'My enrolled courses (current semester)'}
        </h3>
        {filteredCourses.length === 0 ? (
          <Card className="rounded-2xl border border-dashed">
            <CardContent className="py-8 text-center text-muted-foreground">
              <p>{language === 'ar' ? 'لا توجد مواد مسجلة في الفصل الحالي.' : 'No courses in your current semester.'}</p>
              <p className="mt-2 text-sm">{language === 'ar' ? 'اذهب إلى حالاتي عبر الفصول وأضف فصلاً ثم عيّنه كفصل حالي، أو سجّل مواد من الكتالوج أعلاه (ستُضاف للفصل الحالي).' : 'Go to Academic History to add a semester and set it as current, or enroll from the catalog above (courses go to current semester).'}</p>
            </CardContent>
          </Card>
        ) : (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <CardHeader className="p-8 pb-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {(course.withdrawn === 1 || course.withdrawn === true) && (
                    <Badge variant="outline" className="rounded-full text-[10px] border-amber-500 text-amber-600 dark:text-amber-400">W</Badge>
                  )}
