...
export default function AcademicHistory() {
  ...
  const handleAddSemester = async () => {
    const name = formNameOverride.trim() || null;
    const payload = { year_number: formYear, semester_number: formSemester, is_current: formCurrent };
    if (name) payload.name = name;
    try {
      setSaving(true);
      await api.post('/student/semesters', payload);
  ...
  const handleEndSemester = async (id) => {
    if (!confirm(isArabic ? 'إنهاء هذا الفصل؟ لن يمكنك إضافة مواد جديدة إليه.' : 'End this semester? You will not be able to add courses to it.')) return;
    try {
      await api.patch(`/student/semesters/${id}`, { is_ended: true });
      toast.success(isArabic ? 'تم إنهاء الفصل' : 'Semester ended');
      fetchSemesters();
      fetchSummary(id);
    } catch (e) {
      toast.error(e.response?.data?.detail || 'Failed');
  ...
  const unassignedCourses = allCourses.filter((c) => c.semester_id == null && c.withdrawn != 1);
  const carriedCourses = allCourses.filter((c) => c.finalized_at != null && c.passed === 0 && c.withdrawn != 1);
  const assignableCourses = [...unassignedCourses];
  carriedCourses.forEach((c) => {
    if (!assignableCourses.some((a) => a.id === c.id)) assignableCourses.push(c);
  });
  const selectedSemesterEnded = summary?.is_ended === 1 || summary?.is_ended === true;
    ...
    <div className="space-y-6" data-testid="academic-history-page">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-xl font-bold font-display">{t('academicHistory.title')}</h2>
              <Button onClick={() => { setDialogOpen(true); setFormNameOverride(''); setFormCurrent(false); setFormYear(1); setFormSemester(1); }}>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex justify-center">
              <Button onClick={() => { setDialogOpen(true); setFormNameOverride(isArabic ? '2022/2023' : '2022/2023'); setFormCurrent(true); setFormYear(1); setFormSemester(1); }}>
                <Plus className="w-4 h-4 mr-2" />
                {t('academicHistory.addSemester')}
              </Button>
            </div>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={selectedId != null ? String(selectedId) : ''} onValueChange={(v) => setSelectedId(v ? parseInt(v, 10) : null)}>
              <SelectTrigger className="w-full sm:w-[280px] max-w-full">
                ...
                {semesters.map((s) => (
                  <SelectItem key={s.id} value={String(s.id)}>
                    <span className="flex items-center gap-2">
                      ...
                      {(s.is_ended === 1 || s.is_ended === true) ? <Badge variant="outline" className="text-xs">{isArabic ? 'منتهي' : 'Ended'}</Badge> : null}
                    </span>
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            {selectedId && (
              <>
                {!selectedSemesterEnded && (
                  <Button variant="outline" size="sm" onClick={() => handleEndSemester(selectedId)}>
                    {isArabic ? 'إنهاء الفصل' : 'End semester'}
                  </Button>
                )}
          ...
          {summary && (
            <Card>
              <CardHeader className="pb-2 flex flex-row items-center justify-between gap-2 flex-wrap">
                ...
                {(summary.is_ended === 1 || summary.is_ended === true) && (
                  <Badge variant="outline">{isArabic ? 'فصل منتهي' : 'Semester ended'}</Badge>
                )}
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {assignableCourses.length > 0 && !selectedSemesterEnded && (
                      ...
                      <SelectContent>
                        {assignableCourses.map((c) => (
                          <SelectItem key={c.id} value={String(c.id)}>
                            ...
                            {c.finalized_at != null && c.passed === 0 ? ` ${isArabic ? '(محملة)' : '(carried)'}` : ''}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={String(formYear)} onValueChange={(v) => setFormYear(parseInt(v, 10))}>
              ...
              <div>
                <Label>{isArabic ? 'الفصل' : 'Semester'}</Label>
                <Select value={String(formSemester)} onValueChange={(v) => setFormSemester(parseInt(v, 10))}>
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div>
              <Label className="text-muted-foreground">{t('academicHistory.semesterName')} ({isArabic ? 'اختياري' : 'optional'})</Label>
              <Input
                value={formNameOverride}
                onChange={(e) => setFormNameOverride(e.target.value)}
                placeholder={isArabic ? 'مثال: 2022/2023' : 'e.g. 2022/2023'}
                className="mt-1"
              />
            </div>
