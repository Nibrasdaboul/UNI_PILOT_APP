...
// Excludes withdrawn courses from GPA, credits, completed, carried.
function recomputeCumulativeFromCourses(coursesList) {
  const list = (coursesList || []).filter((c) => c.withdrawn != 1 && c.withdrawn !== '1');
  const passed = list.filter((c) => c.finalized_at != null && c.passed === 1);
  const failed = list.filter((c) => c.finalized_at != null && c.passed === 0);
...
app.get('/api/dashboard/summary', authMiddleware, async (req, res) => {
  ...
  const currentSemesterCourses = currentSemId != null
    ? coursesList.filter((c) => Number(c.semester_id) === currentSemId && c.withdrawn != 1)
    : coursesList.filter((c) => c.finalized_at == null && c.withdrawn != 1);
...
app.get('/api/dashboard/chart-data', authMiddleware, async (req, res) => {
  ...
  const semesters = await db.prepare(
    'SELECT id, name, sort_order FROM student_semesters WHERE user_id = ? ORDER BY sort_order ASC, id ASC'
  ).all(userId);
  const coursesList = await db.prepare(`
    SELECT id, semester_id, credit_hours, current_grade, finalized_at, passed, withdrawn
    FROM student_courses WHERE user_id = ?
  `).all(userId);
  const list = (semesters || []).map((s) => {
    const sid = Number(s.id);
    const courses = (coursesList || []).filter(
      (c) => Number(c.semester_id) === sid && c.withdrawn != 1 && c.withdrawn !== '1'
    );
    const finalized = courses.filter((c) => c.finalized_at != null);
    const withMark = finalized.map((c) => ({ ...c, final_mark: c.current_grade }));
    const gpa = computeSemesterGpa(withMark);
    const credits = finalized.filter((c) => c.passed === 1).reduce((sum, c) => sum + (Number(c.credit_hours) || 0), 0);
    return { semester_id: sid, semester_name: s.name || 'Semester', gpa, credits };
  });
...
app.post('/api/notes/improve', authMiddleware, requireAiQuota, async (req, res) => {
  try {
