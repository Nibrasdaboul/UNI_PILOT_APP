...
semestersRouter.get('/', async (req, res) => {
  const rows = await db.prepare(`
    SELECT id, user_id, name, sort_order, is_current, app_rating, created_at
...
semestersRouter.post('/', async (req, res) => {
  const body = req.body || {};
  const name = (body.name || '').trim() || 'فصل جديد';
...
semestersRouter.get('/:id', async (req, res) => {
  ...
  const courses = await db.prepare(`
    SELECT id, course_name, course_code, credit_hours, current_grade, finalized_at, passed
    FROM student_courses WHERE semester_id = ? AND user_id = ?
  `).all(id, req.user.id);
