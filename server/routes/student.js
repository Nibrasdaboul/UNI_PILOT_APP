  ...
  if (onlyCurrent) {
    ...
    if (cur) {
      const rows = await db.prepare(`
        SELECT id, user_id, catalog_course_id, course_name, course_code, credit_hours, semester, difficulty, target_grade, professor_name, description, current_grade, progress, finalized_at, passed, created_at, semester_id
...
studentRouter.patch('/courses/:id', async (req, res) => {
  ...
  if (body.semester_id !== undefined) {
    ...
    if (sid != null) {
      const sem = await db.prepare('SELECT id FROM student_semesters WHERE id = ? AND user_id = ?').get(sid, req.user.id);
